...
async function readCollection(collectionPath, accessToken) {
    ...
    do {
        ...
        if (!resp.ok) { console.error(`Read ${collectionPath} failed: ${resp.status}`); break; }
...
async function run() {
    try {
        ...
        console.log("Fetching workAreaTypes from Firestore...");
        const workAreaTypes = await readCollection("workAreaTypes", token);
        console.log(`Found ${workAreaTypes.length} workAreaTypes in Firestore.`);
        
        let count = 0;
        for (const wa of workAreaTypes) {
            const types = wa.defaultTaskTypes || wa.taskTypeIds || [];
            if (types.length > 0) {
                console.log(`Updating ${wa.name} (${wa._firestoreId}) with ${types.length} tasks...`);
                const { error } = await supabase
                    .from('work_area_types')
                    .update({ default_task_types: types })
                    .eq('firestore_id', wa._firestoreId);
                    
                if (error) console.error("Error updating", wa.name, error);
                else count++;
            }
        }
