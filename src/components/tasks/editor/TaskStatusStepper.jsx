...
const STATUS_COLORS = {
    '#64748b': { text: 'text-white', activeBg: 'bg-slate-500', activeText: 'text-white', ring: 'ring-slate-500' },
    ...
    '#6b7280': { text: 'text-white', activeBg: 'bg-gray-500', activeText: 'text-white', ring: 'ring-gray-500' },
};

export default function TaskStatusStepper({ currentStatus, onStatusChange, canEdit, variant = 'full' }) {
    ...
    const Banner = isSpecialState ? (
        <div className={`flex items-center justify-between px-4 py-2.5 ${isBlocked
            ...
            <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-black uppercase tracking-wider ${isBlocked ? 'text-red-400' : 'text-white'
                    }`}>
    ...
    const Stepper = (
        <div className={`flex items-center gap-0 overflow-x-auto scrollbar-none ${variant === 'inline' ? 'px-2 lg:px-0' : 'py-2 md:py-3 px-3 md:px-4 lg:px-5'}`} style={{ WebkitOverflowScrolling: 'touch' }}>
                ...
                return (
                    <React.Fragment key={s}>
                        <button
                            ...
                            className={`flex items-center gap-1 lg:gap-1.5 px-2 lg:px-2.5 py-1.5 lg:py-1 rounded-lg text-[10px] sm:text-[11px] font-black uppercase tracking-wider whitespace-nowrap transition-all flex-shrink-0 ${isActive
    ...
    if (variant === 'bannerOnly') {
        return isSpecialState ? Banner : null;
    }
