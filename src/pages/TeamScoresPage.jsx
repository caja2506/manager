...
function DeltaBadge({ delta }) {
    if (!delta || delta.score === 0) {
        return <span className="text-[9px] text-slate-600 flex items-center gap-0.5"><Minus size={10} /> 0</span>;
    }
    ...
    return (
        <svg width={size} height={size} className="transform -rotate-90">
            <circle cx={size / 2} cy={size / 2} r={radius} fill="none"
                stroke="rgba(148,163,184,0.12)" strokeWidth={strokeWidth} />
            <circle cx={size / 2} cy={size / 2} r={radius} fill="none"
                stroke={color} strokeWidth={strokeWidth} strokeLinecap="round"
                strokeDasharray={circumference} strokeDashoffset={circumference - progress}
                style={{ transition: 'stroke-dashoffset 1.4s ease-out' }} />
            <text x={size / 2} y={size / 2} textAnchor="middle" dominantBaseline="central"
                className="fill-slate-100 font-bold" style={{ fontSize: size * 0.28, transform: 'rotate(90deg)', transformOrigin: 'center' }}>
                {animatedScore}
            </text>
        </svg>
    );
        ...
        <div className="mb-3">
            <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] text-slate-600">({Math.round(dim.weight * 100)}%)</span>
                </div>
...
function ScoreCard({ person, index, isExpanded, onToggle, history }) {
    ...
    if (person.isManager) {
        return (
            <div className="rounded-2xl p-5 border border-purple-500/20 bg-purple-500/5" style={{
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="text-slate-100 font-semibold text-sm">{person.displayName}</div>
    ...
    if (person.insufficientData) {
        return (
            <div className="rounded-2xl p-5 border border-amber-500/20 bg-amber-500/5" style={{
                animationDelay: `${index * 80}ms`,
            }}>
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg"
    ...
    return (
        <div className={`rounded-2xl border overflow-hidden transition-all duration-300 ${isExpanded ? 'bg-slate-900/80 border-slate-700' : 'bg-slate-900/40 border-slate-800/60'}`} style={{
            borderColor: isExpanded ? `${lvlColor}33` : undefined,
            animationDelay: `${index * 80}ms`,
        }}>
            {/* Header */}
            <button onClick={onToggle} className="w-full p-5 flex items-center gap-4 hover:bg-slate-800/50 transition-colors">
                {/* Role icon */}
                ...
                <div className="text-left flex-1 min-w-0">
                    <div className="text-slate-100 font-semibold text-sm truncate">{person.displayName}</div>
                        ...
                        {dims.map(([key, dim]) => {
                            ...
                            return (
                                <div key={key} className="flex items-start gap-2 text-[11px] leading-relaxed rounded-lg px-3 py-2"
                                    ...
                                    <div>
                                        <span className="text-slate-300 font-semibold">{insight.title}: </span>
...
function SummaryKPI({ label, value, Icon: KpiIcon, color, visible }) {
    ...
    return (
        <div className="rounded-xl p-4 flex items-center gap-3 bg-slate-900/60 border border-slate-800" style={{
            ...
            <div>
                <div className="text-2xl font-bold text-slate-100">{animVal}</div>
...
function TeamOverlayChart({ historyMap, teamScores }) {
    ...
    if (members.length === 0 || Object.keys(historyMap).length === 0) {
        return <div className="text-[10px] text-slate-600 text-center py-6">Sin datos históricos — se acumulan al visitar esta página</div>;
    }
    ...
    if (sortedDates.length < 2) {
        return <div className="text-[10px] text-slate-600 text-center py-6">Mínimo 2 días de datos requeridos</div>;
    }
            ...
            <div className="flex flex-wrap gap-2 mt-2">
                {members.slice(0, 6).map((m, i) => (
                    <div key={m.userId} className="flex items-center gap-1">
                        ...
                        <span className="text-[9px] text-slate-500">{m.displayName?.split(' ')[0] || '?'}</span>
                    </div>
                ))}
            </div>
        </div>
...
function WeeklyBarsChart({ weeklyData, teamScores }) {
    ...
    if (members.length === 0 || Object.keys(weeklyData).length === 0) {
        return <div className="text-[10px] text-slate-600 text-center py-6">Sin datos semanales — se generan automáticamente</div>;
    }
    ...
    if (sortedWeeks.length === 0) {
        return <div className="text-[10px] text-slate-600 text-center py-6">Sin semanas disponibles</div>;
    }
...
function RoleComparisonTable({ teamScores }) {
    ...
    if (roles.length === 0) {
        return <div className="text-[10px] text-slate-600 text-center py-4">Sin datos de roles</div>;
    }
    ...
    return (
        <div className="overflow-x-auto">
            <table className="w-full text-[10px]">
                <thead>
                    <tr>
                        <th className="text-left py-1.5 px-2 text-slate-400 font-bold">Rol</th>
                        <th className="text-center py-1.5 px-2 text-slate-400 font-bold">N</th>
                        <th className="text-center py-1.5 px-2 text-slate-400 font-bold">Score Prom.</th>
                        {Object.keys(DIMENSION_META).map(dim => (
                            <th key={dim} className="text-center py-1.5 px-1 text-slate-500 font-medium">{DIMENSION_META[dim].label.slice(0, 5)}</th>
                        ))}
                    </tr>
                </thead>
                <tbody>
                    {roles.map(([role, data]) => {
                        ...
                        return (
                            <tr key={role} className="border-t border-slate-800">
                                ...
                                <td className="text-center text-slate-400">{data.scores.length}</td>
                                <td className="text-center font-black" style={{ color: scoreColor }}>{avgScore}</td>
                                {Object.keys(DIMENSION_META).map(dim => {
                                    const dimScores = data.dims[dim];
                                    if (!dimScores || dimScores.length === 0) return <td key={dim} className="text-center text-slate-700">—</td>;
...
export default function TeamScoresPage() {
    ...
    return (
        <div className="pb-24">
            ...
            {/* ── Hero Header ── */}
            <div className="relative overflow-hidden border-b border-slate-800" style={{
                background: 'linear-gradient(135deg, rgba(99,102,241,0.06) 0%, rgba(16,185,129,0.03) 50%, rgba(245,158,11,0.03) 100%)',
            }}>
                {/* Ambient orbs */}
                ...
                <div className="relative px-4 sm:px-6 py-8">
                    <div className="flex items-center gap-3 mb-2">
                        <button onClick={() => navigate(-1)} className="p-1.5 rounded-lg hover:bg-slate-800 transition-colors text-slate-500">
                            <ArrowLeft size={18} />
                        </button>
                        <Award size={24} className="text-indigo-400" />
                        <h1 className="text-2xl sm:text-3xl font-bold text-slate-100">Scorecard del Equipo</h1>
                    </div>
                ...
                {roles.map(r => {
                    ...
                    return (
                        <button key={r} onClick={() => setRoleFilter(r)} className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all"
                            style={{
                                background: active ? `${cfg.color}20` : 'rgba(15,23,42,0.6)',
                                color: active ? cfg.color : 'rgb(148,163,184)',
                                border: `1px solid ${active ? `${cfg.color}40` : 'rgb(30,41,59)'}`,
                            }}>
                            {cfg.label}
                        </button>
                    );
            ...
            <section ref={analyticsRef} className="px-4 sm:px-6 mt-6" style={{
                ...
                <div className="bg-slate-900/60 rounded-2xl border border-slate-800 shadow-lg p-6">
                    <div className="flex items-center gap-2 mb-5">
                        <BarChart3 size={18} className="text-indigo-400" />
                        <h3 className="font-bold text-lg text-slate-100">Analítica Temporal</h3>
                        <span className="text-[10px] text-slate-500 ml-auto">Últimas 6 semanas</span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-5">
                        ...
                        <div className="rounded-xl border border-slate-800 p-4 bg-slate-900/40">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <span className="text-xs font-bold text-slate-300">Historial Diario (14 días)</span>
                            </div>
                        ...
                        <div className="rounded-xl border border-slate-800 p-4 bg-slate-900/40">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <span className="text-xs font-bold text-slate-300">Promedios Semanales</span>
                            </div>
                    ...
                    <div className="rounded-xl border border-slate-800 p-4 bg-slate-900/40">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-xs font-bold text-slate-300">Comparación por Rol</span>
                        </div>
