...
export async function createTask(data, userId) {
    ...
    if (data.taskTypeId && !prRequired) {
        try {
            ...
            console.warn('[taskService] PR auto-resolve failed:', err.message);
        }
    }
    ...
    const taskData = createTaskDocument({
        ...
        countsForScore,
        peerReviewRequired: prRequired,
        peerReviewDiscipline: prDiscipline,
        createdBy: userId,
    });
 ...
 * @param {Object} [extraData={}] — additional data for the CF (blockedReason, blockedByUserId, etc.)
...
export async function toggleSubtask(subtaskId, completed, { taskId = null, subtaskTitle = '', userId = null, userName = null, percentComplete = null, totalSubtasks = null, completedSubtasks = null } = {}) {
