...
import { logActivity, ACTIVITY_TYPES } from './activityLogService.firebase';
...
/**
 * Check if currentUserId is the direct supervisor of targetUserId.
 * Checks TWO sources:
 *   1. `reportsTo` field in team member profiles (HR hierarchy)
 *   2. `resourceAssignments` collection (operational engineer → technician)
 *
 * @param {string} currentUserId — the logged-in user
 * @param {string} targetUserId — the user whose timer is being managed
 * @param {Array}  teamMembers  — all team member profiles (with uid/id + reportsTo)
 * @param {Array}  [resourceAssignments=[]] — active resource assignments (engineerId/technicianId)
 * @returns {boolean}
 */
export function isSupervisorOf(currentUserId, targetUserId, teamMembers, resourceAssignments = []) {
    if (!currentUserId || !targetUserId || !teamMembers?.length) return false;
    if (currentUserId === targetUserId) return true; // own timer

    // Check 1: HR hierarchy (reportsTo)
    const target = teamMembers.find(m => (m.uid || m.id) === targetUserId);
    if (target?.reportsTo === currentUserId) return true;

    // Check 2: Operational hierarchy (resourceAssignments)
    if (resourceAssignments?.length > 0) {
        const isEngineerOfTarget = resourceAssignments.some(
            a => a.engineerId === currentUserId && a.technicianId === targetUserId && a.active !== false
        );
        if (isEngineerOfTarget) return true;
    }
 ...
 * Queries Firestore directly — no local state dependency, no permission checks.
 * Use this when you MUST guarantee the timer stops (e.g. status transitions).
 */
export async function forceStopTaskTimers(taskId, reason = 'auto') {
    ...
    try {
        ...
        for (const docSnap of snap.docs) {
            try {
                await stopTimer(docSnap.id, { notes: `Auto-detenido: ${reason}` });
                stopped++;
            } catch (err) {
                console.warn(`[timeService] forceStop failed for ${docSnap.id}:`, err.message);
            }
        }
        return stopped;
    } catch (err) {
        console.error('[timeService] forceStopTaskTimers query failed:', err);
        return 0;
    }
}
...
export async function handleTaskStatusTimerSync({
    ...
    if (['completed', 'cancelled', 'blocked', 'pending', 'backlog'].includes(newStatus)) {
        ...
        // Fallback: if local timeLogs didn't have it, query Firestore directly
        if (!activeLog && taskId) {
            try {
                const q = query(
                    collection(db, COLLECTIONS.TIME_LOGS),
                ...
                const snap = await getDocs(q);
                if (!snap.empty) {
                    activeLog = { id: snap.docs[0].id, ...snap.docs[0].data() };
                }
            } catch (err) {
                console.warn('[timeService] Firestore fallback timer lookup failed:', err.message);
            }
        }
